...
public class Memo1BankApp {
	...
	/*
	* Devuelve una entidad de tipo Account. De lo contrario los tests no tendrían acceso a la
	* cuenta una vez modificado su balance.
	 */
	@PostMapping("/accounts/{cbu}/transactions")
	@ResponseStatus(HttpStatus.CREATED)
	public Account createTransaction(@PathVariable Long cbu, @RequestBody Transaction transaction) {
		return transactionService.createTransaction(transaction, cbu);
	}
